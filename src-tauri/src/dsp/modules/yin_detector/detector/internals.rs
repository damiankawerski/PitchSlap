...
use super::super::utils::buffer::ComplexComponent;
use super::super::utils::buffer::{copy_complex_to_real, copy_real_to_complex, modulus_squared, square_sum, BufferPool};
use super::super::utils::peak::{choose_peak, correct_peak, detect_peaks, PeakCorrection};
    ...
    T: Float,
{
    ...
    let mut signal_complex_guard = ref1.lock().unwrap();
    let mut scratch_guard = ref2.lock().unwrap();
    let signal_complex = &mut signal_complex_guard[..];
    ...
    T: Float + std::iter::Sum,
{
    ...
    let mut scratch_guard = scratch_ref.lock().unwrap();
    let scratch = &mut scratch_guard[..];
    ...
    T: Float + std::iter::Sum,
{
    ...
    let mut signal_complex_guard = scratch_ref1.lock().unwrap();
    let mut truncated_signal_complex_guard = scratch_ref2.lock().unwrap();
    let mut scratch_guard = scratch_ref3.lock().unwrap();
    let signal_complex = &mut signal_complex_guard[..signal.len()];
    let truncated_signal_complex = &mut truncated_signal_complex_guard[..signal.len()];
    let scratch = &mut scratch_guard[..signal.len()];
