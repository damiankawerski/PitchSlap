...
impl AutoTune {
    ...
    fn calculate_shift(&self, detected_freq: f32) -> f32 {
        ...
        // 2. Snap to C Major Scale (C, D, E, F, G, A, B)
        // C = 0, C# = 1, ... relative to C (MIDI 60)
        let note_in_octave = (note_num.round() as i32 % 12 + 12) % 12;
        
        // C Major scale mask: 1 = strict, 0 = avoid
        // 0(C), 2(D), 4(E), 5(F), 7(G), 9(A), 11(B)
        let is_in_scale = matches!(note_in_octave, 0 | 2 | 4 | 5 | 7 | 9 | 11);
        ...
        let target_note = if is_in_scale {
             ...
             for offset in check_offsets {
                 ...
                 if matches!(cand_oct, 0 | 2 | 4 | 5 | 7 | 9 | 11) {
