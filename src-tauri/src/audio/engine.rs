...
impl AudioEngine {
    pub fn new(input_device: &AudioDevice, output_device: &AudioDevice, opt: &AudioDeviceOptions) -> anyhow::Result<Self> {
        ...
        let streams = AudioStreams::new(&input_device, &output_device, latency_samples)?;
        Ok(AudioEngine {
            streams,
        })
    }
